...
describe("parseSkillRequest", () => {
  ...
  it("keeps an answer the requester wrapped onto the next line", () => {
    // Issue #13 was written this way, and reading only the first line left the
    // generator a condition with no consequence to write an eval against.
    const result = parseSkillRequest(
      body({
        "Example scenarios and expected results":
          "Scenario: user provides a work item link\nExpected: if the INVEST average falls below 4\nit is not ready",
      }),
    );

    expect(result.ok && result.request.scenarios).toEqual([
      {
        scenario: "user provides a work item link",
        expected: "if the INVEST average falls below 4\nit is not ready",
      },
